...
function Personal() {
  ...
  const handleSelectChange = (event) => {
    ...
    const empleadoSeleccionado = empleados.find(empleado => empleado.Puesto === event.target.value);
  ...
  const handleUpdate = async () => {
    try {
      await updatePlaceRequest(iPuesto, { "Puesto": iPuesto, "Empleado": iEmpleado, "Planta": iPlanta });
  ...
  const handleCreate = async () => {
    if (!newPuesto || !newEmpleado || !newPlanta) {
      alert('Por favor, rellena todos los campos antes de crear un nuevo puesto.');
      return;
    }

    try {
      await createPlaceRequest({ "Puesto": newPuesto, "Empleado": newEmpleado, "Planta": newPlanta });
  ...
  const handleDelete = async () => {
    const confirmation = window.confirm('¿Estás seguro de que quieres eliminar este puesto?');
    if (confirmation) {
      try {
        await deletePlaceRequest(iPuesto, { "Puesto": iPuesto, "Empleado": iEmpleado, "Planta": iPlanta });
    ...
    <div className="main-container">
      ...
      <div className="personal-container">
        <div className="left-section">
          <select className="personal-select" value={selectedOption} onChange={handleSelectChange}>
          ...
          <form className="form-personal">
            <label className="label-personal">
              Puesto: <input className="input-personal" type="text" name="iPuesto" value={iPuesto} onChange={handleInputChange} placeholder="Puesto" readOnly />
            </label>
            <label className="label-personal">
              Empleado: <input className="input-personal" type="text" name="iEmpleado" value={iEmpleado} onChange={handleInputChange} placeholder="Empleado" />
            </label>
            <label className="label-personal">
              Planta:
              <select className="select-personal" name="iPlanta" value={iPlanta} onChange={handleInputChange}>
          ...
          <div className="div-botones-personal" >
            <button className="modificar-personal" onClick={handleUpdate}>Modificar puesto</button>
            <button className="agregar-personal" onClick={() => setShowModal(true)}>Añadir nuevo puesto</button>
            {showModal && (
              <div className="modal">
                <div className="modal-content">
                  <h2 className="modal-h2">Añadir nuevo puesto</h2>
                  <input className="input-modal" type="text" value={newPuesto} onChange={(e) => setNewPuesto(e.target.value)} placeholder="Puesto (Número)" />
                  <input className="input-modal" type="text" value={newEmpleado} onChange={(e) => setNewEmpleado(e.target.value)} placeholder="Empleado" />
                  <select className="input-modal" value={newPlanta} onChange={(e) => setNewPlanta(e.target.value)}>
                    <option value="Baja">Baja</option>
                    <option value="Laboratorio">Laboratorio</option>
                  </select>
                  <button className="modal-crear" onClick={handleCreate}>Crear</button>
                  <button className="modal-cancelar" onClick={() => { setShowModal(false); setNewPuesto(''); setNewEmpleado(''); setNewPlanta(''); }}>Cancelar</button>
                </div>
              </div>
            )}
            <button className="eliminar-personal" onClick={handleDelete}>Eliminar puesto</button>
          </div>
          <div>
            <Link to="/home">
        ...
        <div className="right-section">
          <img className="imagen" src={selectedImage} alt="Planta seleccionada" />
          <button className="controlador-imagen" onClick={() => setSelectedImage(Plano_P1)}>Planta Baja</button>
          <button className="controlador-imagen" onClick={() => setSelectedImage(Plano_P0)}>Laboratorio</button>
        </div>
      </div>
    </div>
  );
